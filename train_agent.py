...
class ACDataLoader(object):
    def __init__(self, head_entity_id, batch_size):
        self.head_entity_id = np.array(head_entity_id)
        self.num_head_entity = len(head_entity_id)
        self.batch_size = batch_size
        self.reset()

    def reset(self):
        self._rand_perm = np.random.permutation(self.num_head_entity)
        self._start_idx = 0
        self._has_next = True
    ...
    def get_batch(self):
        ...
        end_idx = min(self._start_idx + self.batch_size, self.num_head_entity)
        batch_idx = self._rand_perm[self._start_idx:end_idx]
        batch_head_entity_id = self.head_entity_id[batch_idx]
        self._has_next = self._has_next and end_idx < self.num_head_entity
        self._start_idx = end_idx
        return batch_head_entity_id.tolist()


def train(args):
    env = BatchKGEnvironment(args)
    head_entity_id = list(env.kg(args.head_entity).keys())
    dataloader = ACDataLoader(head_entity_id, args.batch_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while dataloader.has_next():
            batch_head_entity_id = dataloader.get_batch()
            ### Start batch episodes ###
            batch_state = env.reset(batch_head_entity_id)  # numpy array of [bs, state_dim]
            done = False
            while not done:
            ...
            lr = args.lr * max(1e-4, 1.0 - float(step) / (args.epochs * len(head_entity_id) / args.batch_size))
        ...
        ### END of epoch ###
        if epoch % 5 ==0:
            policy_file = '{}/model/policy_model_epoch_{}.ckpt'.format(args.log_dir, epoch)
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='/home/linjie/projects/KG/PGPR_hetionet/output/hetionet_CmC',
