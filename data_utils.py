...
class LoadDataset(object):
    ...
    def load_entities(self):
        """Load 11 global entities from data files:
        `Anatomy`, `BiologicalProcess`, `CellularComponent`, `Compound`, `Disease`, `Gene`,
        `MolecularFunction`,`Pathway`,`PharmacologicClass`,`SideEffect`,`Symptom`.
        ...
        entities_group = entities_df.groupby('metanode')
        for metanode, group in entities_group:
            setattr(self, metanode, edict(vocab=list(group['entities_value']), vocab_size=len(group)))
            print(f"load {metanode} of size {len(group)}")
    ...
    def load_target_entity_relations(self):
        """Load 25 product -> ? relations:
        - `AdG`,`AeG`, `AuG`:  Anatomy -> Gene,
        - `CbG`, `CdG`, `CuG`: Compound -> Gene,
        - `CcSE`:              Compound -> SideEffect
        - `CpD`, `CtD`:        Compound -> Disease,
        - `CrC`, `CmC`:        Compound -> Compound
        - `DaG`, `DdG`, `DuG`: Disease -> Gene
        - `DlA`:               Disease -> Anatomy,
        - `DpS`:               Disease -> Symptom,
        - `DrD`:               Disease -> Disease,
        - `GcG`, `GiG`, `GrG`: Gene -> Gene
